...
source: "https://rockthejvm.com/articles/http4s-unleashing-the-power-of-http-apis-library"
author:
  - "[[Rock the JVM]]"
published: 2021-06-07
created: 2025-03-31
description: "Master functional programming basics: use http4s with the Cats ecosystem to seamlessly create powerful HTTP APIs"
tags:
  - "clippings"
  - "scala"
---
...
The route we are matching contains some query parameters. **We can introduce the match of query parameters using the `:?` extractor**. Even though there are many ways of extracting query params, the library sponsors the use of *matchers*. In detail, extending the `abstract class QueryParamDecoderMatcher`, we can pull directly into a variable a given query parameter:
...
The trick is that the type `HttpRoutes[F]`, being an instance of the `Kleisli` type, is also a [`Semigroup`](https://rockthejvm.com/articles/semigroups-and-monoids-in-scala). In fact, it’s a `SemigroupK`, as we have a semigroup of an effect (remember the `F[_]` type constructor).
...
Suppose that we want to implement some type of validation on a query parameter, as returning a *Bad Request* HTTP Status if the query parameter `year` doesn’t represent a positive number. First, we need to change the type of matcher we need to use, introducing the validation:
...
The `CIString` type comes from the Typelevel ecosystem ( `org.typelevel.ci.CIString` ), and represents a *case-insensitive* string.
...
The above code uses the *for-comprehension* syntax making it more readable. However, it is possible only because we initially import the Cats extension methods of `cats.syntax.flatMap._` and `cats.syntax.functor._`. Remember that an implicit type class `Monad[F]` must be defined for’ F’.
...
If we want the substitution model to apply to the whole part of the application, the only appropriate place to call the `unsafeRunSync` is in the `main` method, also called *the end of the world*. Indeed, it’s precisely what the `IOApp.run` method does for us.
